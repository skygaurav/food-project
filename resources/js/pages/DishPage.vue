  ...
  <section class="space-y-10">
    ...
    <section>
      ...
      <div class="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <div v-for="dishItem in relatedDishes" :key="dishItem.name" class="rounded border border-slate-200 p-3">
          <img :src="dishItem.image" :alt="dishItem.name" class="h-32 w-full rounded object-cover" />
          <p class="mt-2 text-sm font-semibold">{{ dishItem.name }}</p>
          <p class="text-xs text-slate-500">{{ dishItem.restaurant }}</p>
        </div>
      </div>
    </section>
  </section>
